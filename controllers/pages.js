...
export const getPageById = (req, res) => {
  ...
  db.query(sqlPageId, [pageTitle], (err, pageResult) => {
    ...
    const sql =
      "SELECT * FROM pages JOIN sections_page ON pages.idPage = sections_page.idPage WHERE pages.idPage = ?";
...
export const addPage = (req, res) => {
  ...
  db.query(sqlPageId, [sectionBody.link], (err, pageResult) => {
    ...
    const sqlSection =
      "INSERT INTO departement_ich.sections_page (idSection, texte, ordre_positionnement, idPage) VALUES (?, ?, ?, ?)";
