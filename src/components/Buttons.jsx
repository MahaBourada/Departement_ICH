...
export const SmallFilledButton = ({ type, text, onClick }) => {
  return (
    <button
      type={type}
      onClick={onClick}
      className={`cursor-pointer font-main dyslexiaTheme:font-dyslexia font-medium rounded-xl h-fit px-5 py-3 mx-3 my-1 text-black bg-accent transition-colors duration-300 hover:bg-hover-accent dark:bg-dark-accent dark:hover:bg-dark-hover-accent dark:text-dark-white max-sm:w-fit max-md:w-42 max-md:mb-4 text-nav max-sm:text-[1.125rem] leading-normal`}
    >
      {text}
    </button>
  );
...
export const BigFilledButton = ({ type, text }) => {
  return (
    <button
      type={type}
      className={`cursor-pointer text-black font-main dyslexiaTheme:font-dyslexia font-medium rounded-xl px-5 py-3 my-3 bg-accent transition-colors duration-300 hover:bg-hover-accent dark:bg-dark-accent dark:hover:bg-dark-hover-accent dark:text-dark-white w-full text-nav max-sm:text-[1.125rem] leading-normal`}
    >
      {text}
    </button>
  );
